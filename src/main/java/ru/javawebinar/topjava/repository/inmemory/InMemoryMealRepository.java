...
public class InMemoryMealRepository implements MealRepository {
    ...
    public List<Meal> getAll(int userId) {
        Map<Integer, Meal> mRepository = repository.get(userId);
        return CollectionUtils.isEmpty(mRepository) ? Collections.emptyList() : mRepository.values().stream()
                .sorted(Comparator.comparing(Meal::getDateTime).reversed())
                .collect(Collectors.toList());
    }

    @Override
    public List<Meal> getAllFiltered(int userId, LocalDateTime dateTimeMin, LocalDateTime dateTimeMax) {
        Map<Integer, Meal> mRepository = repository.get(userId);
        return CollectionUtils.isEmpty(mRepository) ? Collections.emptyList() : mRepository.values().stream()
                .filter(meal -> meal.getDateTime().isBefore(dateTimeMax) && meal.getDateTime().isAfter(dateTimeMin))
                .sorted(Comparator.comparing(Meal::getDateTime).reversed())
                .collect(Collectors.toList());
    }
